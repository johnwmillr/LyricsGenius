...
class Genius(API, PublicAPI):
    ...
    def search_album(self, title=None, artist="",
                     album_id=None, get_full_info=True, text_format=None):
        """Searches for a specific album and gets its songs.

        You must pass either a :obj:`title` or an :obj:`album_id`.

        Args:
            title (:obj:`str`, optional): Album title to search for.
        ...
        Examples:
            .. code:: python
                ...
                genius = Genius(token)
                artist = genius.search_artist('MGK', max_songs=0)
                album = genius.search_album('Hotel Diablo', artist.name)
                print(album.title)

        """
        msg = "You must pass either a `title` or an `album_id`."
        assert any([title, album_id]), msg
        ...
        if album_id:
            ...
            album_info = self._get_item_from_search_response(response, title,
                                                             type_="album",
                                                             result_type="name")
    ...
    def search_song(self, title=None, artist="", song_id=None,
                    get_full_info=True, public_api=False):
        ...
        if song_id:
            result = self.song(song_id, public_api=public_api)['song']
        else:
            search_term = "{s} {a}".format(s=title, a=artist).strip()
        ...
        if song_id is None and get_full_info is True:
            new_info = self.song(song_id, public_api=public_api)['song']
    ...
    def search_artist(self, artist_name, max_songs=None,
        ...
        artist_info = self.artist(artist_id, public_api=public_api)['artist']
        ...
        while not reached_max_songs:
            ...
            for song_info in songs_on_page['songs']:
                ...
                lyrics = self.lyrics(song_info['url'])
                if get_full_info:
                    new_info = self.song(song_info['id'], public_api=public_api)['song']
                    song_info.update(new_info)
                song = Song(self, song_info, lyrics)
